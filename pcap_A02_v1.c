...
//START
	while(1){
		...
		if(ck_packet==0){
			...
			_ip=(struct in_addr*)(&(packet[SIZE_ETHER]));
...
*/
			...
			*/
			printf("=====================================================\n");
			printf("dst MAC : %s\n", ether_ntoa((struct _eth_set*) _eth->dst_eth_addr));
			printf("src MAC : %s\n", ether_ntoa((struct _eth_set*) _eth->src_eth_addr));
			printf("Type    : %04x\n\n", ntohs(_eth->eth_type));
